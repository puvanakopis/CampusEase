...
from app.schemas.accommodation_schema import AccommodationResponse, AccommodationReview,  UserResponse
    ...
    for acc_id in user_doc.get("save_accommodations", []):
        ...
        if acc_doc:
            ...
            for rev in acc_doc.get("reviews", []):
                ...
                reviews.append(AccommodationReview(user=user_obj.dict() if user_obj else None, **rev))

        
        acc_doc_copy = acc_doc.copy()
        acc_doc_copy.pop("reviews", None)

        accommodations.append(AccommodationResponse(**acc_doc_copy, reviews=reviews, owner=None))
        ...
        for veh_id in user_doc.get("save_transports", []):
            ...
            if veh_doc:
                ...
                for rev in veh_doc.get("reviews", []):
                    user_obj = await get_user_by_id(rev.get("user_id"))
                reviews.append(VehicleReview(user=user_obj.dict() if user_obj else None, **rev))        
                
                veh_doc_copy = veh_doc.copy()
                veh_doc_copy.pop("reviews", None)

        vehicles.append(VehicleResponse(**veh_doc_copy, reviews=reviews, owner=None))
    return {
        "success": True,
        "status_code": 200,
